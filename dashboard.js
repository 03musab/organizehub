...
class UIManager {
  ...
  handleAddTodo() {
      ...
      const priority = this.priorityDropdown.value;
      if (task === "") {
  ...
  displayTodos(todos) {
      ...
      if (todos.length === 0) {
          this.todosListBody.innerHTML = `<tr><td colspan="6" class="text-center">No task found</td></tr>`;
          return;
      }
      ...
      todos.forEach((todo) => {
        ...
        <tr class="todo-item" data-id="${todo.id}">
          ...
          <td>${this.todoItemFormatter.formatPriority(todo.priority)}</td>
          <td>
  ...
  handleEditTodo(id) {
      ...
      if (todo) {
          ...
          this.priorityDropdown.value = todo.priority;
